...
object FlinkAssignment {
  ...
  //    input.assignTimestampsAndWatermarks(new AssignerWithPeriodicWatermarks[Commit] {
  ...
  //    })
  //      .map(c => (new DateTime(c.commit.committer.date).toString("dd-MM-yyyy"), 1))
